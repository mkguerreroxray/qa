...
public class ES_Web_Agent_Money_Transfer_with_Existent_SenderTest extends TestBase {
	...
	public void ES_Web_Agent_Money_Transfer_with_Existent_Customer_Test() throws Exception {
		...
		action.moveToElement(SenderModule.getSourceOfFunds()).click().sendKeys("Gift").wait(1000);
		action.moveToElement(SenderModule.getSourceOfFunds()).sendKeys(Keys.ENTER);

		// PURPOSE OF TRANSACTION
		action.moveToElement(SenderModule.getPurposeOfTransaction()).click().sendKeys("Gift").wait(1000);
		action.moveToElement(SenderModule.getPurposeOfTransaction()).sendKeys(Keys.ENTER);
