...
public class SenderModuleItaly extends TestBase {

	randomNumberGenarator RndmNmb = new randomNumberGenarator();
	randomMailGenerator RndmMail = new randomMailGenerator();

	@Test
	public void SenderModuleItalyTest() throws InterruptedException, IOException {
		...
		// DOC. AUTHORITY
		DropDownSelect.DropdownListSelect(SenderModule.getDocAuthority(), "Comune");

		// OCCUPATION
		DropDownSelect.DropdownListSelect(SenderModule.getOccupation(), "Student");

		// ANNUAL INCOME
		DropDownSelect.DropdownListSelect(SenderModule.getAnnualIncome(), "10.000 - 20.000 €");

		// FISCAL CODE
		SenderModule.getFiscalCode().click();
		...
		// RESIDENCE CITY
		SenderModule.getResidenceCity().sendKeys("Roma");
		Thread.sleep(3000);
